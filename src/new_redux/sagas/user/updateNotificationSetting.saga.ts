...
function* updateNotificationSettingRequestSaga(
  action: PayloadAction<AppActionTypes.User.IUpdateNotificationSettingRequestAction>
) {
  try {
    yield put(AppActions.loading.setLoading());

    const result: ResponseGenerator = yield call(async () => {
      return await makeAPIRequst("user/notification", "PUT", action.payload.notificationSetting);
    });
...
export default (function* () {
  yield takeLatest("user/updateNotificationSettingRequest", updateNotificationSettingRequestSaga);
})();
