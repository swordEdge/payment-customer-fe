...
function* verifyGATokenRequestSaga(
  action: PayloadAction<AppActionTypes.User.IVerifyGATokenRequestAction>
) {
  try {
    yield put(AppActions.loading.setLoading());

    const result: ResponseGenerator = yield call(async () => {
      return await makeAPIRequst("user/ga-verify", "POST", { gaCode: action.payload.gaCode });
    });

    yield put(AppActions.loading.finishLoading());
    yield put(AppActions.user.verifyGATokenSuccess(result.data));
...
export default (function* () {
  yield takeLatest("user/verifyGATokenRequest", verifyGATokenRequestSaga);
})();
