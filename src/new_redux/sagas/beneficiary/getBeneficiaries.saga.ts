...
function* getBeneficiariesRequestSaga(
	action: PayloadAction<AppActionTypes.Beneficiary.IGetBeneficiariesRequestAction>
) {
	try {
		yield put(AppActions.loading.setLoading());

		const result: ResponseGenerator = yield call(async () => {
			return await makeAPIRequst("beneficiary", "GET");
		});
...
export default (function* () {
	yield takeLatest(
		"beneficiary/getBeneficiariesRequest",
		getBeneficiariesRequestSaga
	);
})();
