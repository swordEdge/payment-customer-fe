...
import { ChangePinModelStyle } from './index.style';

type SuccessChangePinModelProps = {
  title: string;
  onCancel: () => void;
  onConfirm: () => void;
  receiveName: string;
...
export const SuccessChangePinModalComponent: React.FC<
  SuccessChangePinModelProps
> = ({ onCancel, onConfirm, receiveName }) => {
  const onCancelClick = () => {
    onCancel();
  };
  ...
  return (
    <ChangePinModelStyle>
      <Box className='success-model-container'>
        ...
        <Box className='success-model-content'>Payment Successful!</Box>

        <Box className='success-model-footer'>
          Congratulations, your transaction is completed and was successfully
          paid to {receiveName}.
        </Box>
      </Box>
    </ChangePinModelStyle>
